...
module.exports.leave = async function (interaction) {
  ...
  if (cooldown.has(`${guildId}_${memberId}`)) {
    ...
    if (!userVoiceChannel) {
      const embed = new MessageEmbed()
        ...
        .setTitle("Warning")
        .setDescription(
          `To avoid bad actors, you must join the VC to leave. Join ${queue.voiceChannel.name} and try again.`
        );
      await interaction.reply({ embeds: [embed] });
    } else if (!queue) {
      const embed = new MessageEmbed()
      ...
      distube.distube.voices.leave(guildId);
      const embed = new MessageEmbed()
        .setColor("#fda238")
        .setTitle("Success")
        .setDescription("Left the VC, see ya bitches!")
