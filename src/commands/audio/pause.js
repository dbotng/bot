...
module.exports.pause = async function (interaction) {
  ...
  if (cooldown.has(`${guildId}_${memberId}`)) {
    ...
    if (!userVoiceChannel) {
      const embed = new MessageEmbed()
        ...
        .setTitle("Warning")
        .setDescription(
          `To avoid bad actors, you must join the VC to pause. Join ${queue.voiceChannel.name} and try again.`
        );
      await interaction.reply({ embeds: [embed] });
    } else if (!queue) {
      const embed = new MessageEmbed()
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          "You cannot pause a radio right? Switch to audio mode and try again!"
        );
      await interaction.reply({ embeds: [embed] });
    } else {
      distube.distube.pause(guildId);
      const embed = new MessageEmbed()
        .setColor("#fda238")
        .setTitle("Success")
        .setDescription("Audio is paused")
